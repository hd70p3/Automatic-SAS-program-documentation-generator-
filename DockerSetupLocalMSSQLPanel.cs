...
{
    public partial class DockerSetupLocalMSSQLPanel : UserControl
    {
        private Label lblTitle, lblServer, lblDatabase, lblUser, lblPassword, lblAppPort, lblDirectory;
        private TextBox textBoxServer, textBoxDatabase, textBoxUser, textBoxPassword, textBoxAppPort, textBoxDirectory;
        ...
        public DockerSetupLocalMSSQLPanel()
        {
        ...
        private void InitializeDockerComponent()
        {
            ...
            lblTitle = new Label
            {
                Text = "Docker App (SQL Login)\r\nConfigure Engrafo to use a MSSQL Server instance.",
            ...
            // Server
            lblServer = new Label { Text = "SQL Server Name:", AutoSize = true };
            textBoxServer = new TextBox { PlaceholderText = @"localhost", Width = 220, Text = @"localhost" };
            tbl.Controls.Add(lblServer, 0, 1);
            tbl.Controls.Add(textBoxServer, 1, 1);

            // Database Name
            lblDatabase = new Label { Text = "Database Name:", AutoSize = true };
            textBoxDatabase = new TextBox { PlaceholderText = "Engrafo_DB", Width = 180, Text = "Engrafo_DB" };
            tbl.Controls.Add(lblDatabase, 0, 2);
            tbl.Controls.Add(textBoxDatabase, 1, 2);

            // SQL User
            lblUser = new Label { Text = "SQL User:", AutoSize = true };
            textBoxUser = new TextBox { PlaceholderText = "e.g. sa", Width = 120 };
            tbl.Controls.Add(lblUser, 0, 3);
            tbl.Controls.Add(textBoxUser, 1, 3);
            ...
            tbl.Controls.Add(lblPassword, 0, 4);
            tbl.Controls.Add(textBoxPassword, 1, 4);
            ...
            // Help icons
            AddHelp(textBoxServer, "The local SQL Server instance name (e.g. localhost, (local) or (local)\\SQLEXPRESS).");
            AddHelp(textBoxDatabase, "The name of the Engrafo database (will be created on first run if not exists).");
            AddHelp(textBoxUser, "Your SQL Server username.");
            AddHelp(textBoxPassword, "Your SQL Server password.");
            AddHelp(buttonBrowse, "The folder to mount for uploads.");
        }
        ...
        private async void buttonDeploy_Click(object sender, EventArgs e)
        {
            ...
            // Build SQL connection string (test)
            var testConnString =
                $"Server={textBoxServer.Text.Trim()};Database=master;User ID={textBoxUser.Text.Trim()};Password={textBoxPassword.Text};TrustServerCertificate=True;Connection Timeout=4;";
            ...
            if (!tbl.Controls.ContainsKey("lblStatus"))
            {
                ...
                {
                    Name = "lblStatus",
                    Text = "Checking SQL login..." + testConnString,
                    AutoSize = true,
            ...
            else
            {
                ...
                lblStatus.Text = "Checking SQL login... " + testConnString;
            ...
            // Compose connection string for the app
            var appConnString =
                $"Server={textBoxServer.Text.Trim()};Database={textBoxDatabase.Text.Trim()};User ID={textBoxUser.Text.Trim()};Password={textBoxPassword.Text};TrustServerCertificate=True;";

            // Generate docker-compose.yml
            try
            {
                var tpl = ReadEmbeddedResource("docker-compose.localmssql.template.yml");
